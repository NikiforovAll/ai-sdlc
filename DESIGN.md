...
**The bar's state is addressable, and the address bar is the only place it is stated.** Every reading a reader can reach by clicking — the view, the lens, the open drawer panel — is written into the hash as `#view=grid&role=lead&open=act-write-spec`, so a coach can paste "look at this activity, filtered to your role" into a chat and the reader lands in it. It is written with `replaceState` and never pushed: Back still leaves the page rather than walking three filter clicks backwards. A default is said by silence — FLOW, every role, and a closed drawer each drop their key — so a link carries only what distinguishes this reading from the one the document opens on, and a key the document cannot answer drops itself, leaving the address bar naming exactly what is on screen.

There is deliberately **no copy-link control**. The bar has two groups and a copy button belongs to neither; it would state no process fact, which is what the projection rule keeps off the page; and a copy needs a success message this world has no vocabulary for — there is no toast, no banner, and a motion budget of two transitions and one loop. Once the hash is written on every change, the browser's own address bar is the affordance, and every reader already has it. This is navigation state only: nothing about annotate mode is ever encoded, because it is a write path and it does not survive into a built page at all.
...
### Annotate Mode
Served pages only, and the one mode this world has. The masthead's quiet end carries its switch — a pencil glyph and the word ANNOTATE in the repo mark's own line, lit green while armed — and `a` arms it from the keyboard. Armed, a 2px signal-green frame rings the viewport with the accent breathing inward, an ink bar sits at the bottom-left corner on the constraint chip's own 14px inset (green border, green glow, the ANNOTATE tab seated in a 5px gutter, the hint beside it), and every node the model names answers the pointer: solid accent outline on hover, dashed ink where a note already exists, solid ink with a heavier wash on the composer's subject. The composer is a floating `.module` placed against its node. Under its head it carries a trail — the drawer's crumb turned into buttons, same 9px uppercase accent and same `→`, the chosen step in bold ink — naming every model node the click sat inside, outermost first, so a note on a shared catalog entry says which activity it was written under and the reader can move the anchor up a step instead of aiming a second click. The trail is absent when the click named one thing and nothing above it. Under it, when the node already carries notes, a washi band lists them — id in ink, the sentence in prose, and the same chain repeated beneath each one as a muted micro-caption on its own line, because two notes on one shared entry are usually about two different uses of it. Trail and caption are one idea said in two places: where this sits. Nothing about the mode survives into a built page.
