...
> **Schema status.** The current schema still carries the free-text v2 fill (`{harness, tooling, level, refs?}`). The `tools:` catalog and the `{tool, level, usage?}` fill are decided (D15) and land with the `processes:` migration (D14) as one model-v3 build step; this doc describes the target model.
