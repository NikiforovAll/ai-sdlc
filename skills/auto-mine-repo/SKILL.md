---
name: auto-mine-repo
description: Mine an ai-sdlc team document out of a repository that already describes its own process, unattended. Nobody is interviewed.
...
The run is **unattended**: inventory, read, author, verify and report without checking in. It stops in exactly one place — the three decisions below, none of which the source can settle — and it resumes the moment they are answered. Anything else that feels like a question is a call you make from the evidence and name in the report.
...
The output is not the end of the work. It is the thing a coach reads *at* a team so they can cut it — which is the `mapping-session` skill's job, starting from a folder that already exists. A mined document that is never taken to a team describes a package; only the team can turn it into a description of a team.
...
```bash
find <pkg> -maxdepth 4 -type f \( -path '*/skills/*' -o -path '*/agents/*' \
  -o -path '*/commands/*' -o -path '*/hooks/*' -o -path '*/workflows/*' \) | sort
cat <pkg>/README.md <pkg>/CHANGELOG.md 2>/dev/null
ls <pkg>/*plugin*/*.json 2>/dev/null
```

Keep the paths, not the file names alone: a package with both `skills/` and `agents/` gives one undifferentiated list otherwise, and `.claude/skills/<name>/SKILL.md` is the layout most of these packages use.
...
This is the one stop in the run. Three calls change the finished picture more than anything else, and **none of them is derivable from the source**. Put all three to the user in one message, with your recommendation, before you write a process file — then finish the document without stopping again:
...
Order is derived from artifacts, so a package's loops need modelling rather than drawing — `references/inference-rules.md` says how.
...
ai-sdlc status <dir>                       # no dangling ids, nothing unreferenced
ai-sdlc export <dir> --out <dir>.html      # the build prints the page count: processes + 2
```

`status` is the one that catches mining mistakes specifically. A dangling id renders silently — the cheatsheet's *Verifying* section says which kinds fail loudly — and an unreferenced catalog entry is the other direction: a tool you inventoried and never placed, which means either an activity is missing or the entry is not part of this document. `--out` names the one-file export, which is the thing you hand over.
...
5. That the next step is `mapping-session` against this folder, with the team in the room.
